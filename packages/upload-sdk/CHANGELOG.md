...
- **分层配置快照**: 新增任务级配置快照机制，解决全局单例模式下组件切换导致的配置混乱问题
    - **全局配置**（`token`、`serverUrl`、`chunkSize` 等）：更新后所有任务（包括队列中的）立即使用新值
    - **任务配置**（`hooks`、`apiPaths`）：在任务创建时保存快照，只对新创建的任务生效
    - 队列中的任务仍使用创建时的配置快照，保证任务执行时的上下文一致性
    - 支持 Token 动态刷新，队列中的任务自动使用新 Token
    - 支持组件切换场景（如 datasetId 变化），旧任务不会错误地使用新配置
    - 完全向后兼容，不影响现有 API
    - 性能影响可忽略（每任务额外 ~200 bytes）
