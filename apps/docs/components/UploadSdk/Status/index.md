...
| **`queued`**      | 队列中 | 已进入队列，等待并发控制器分配资源。          | <span style="color: #8c8c8c">灰色 Tag / ⏳ 排队中</span>     |
...
| **`checking`**    | 连接中 | 点击上传后，正在进行 Init(hook) 和 秒传检测。 | <span style="color: #1890ff">蓝色 Spinner / 📡 连接中</span> |
